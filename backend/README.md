...
- **认证**: bcrypt 密码哈希 + JWT（7 天过期）+ 全路由门禁
...
│   │   └── master_agent.py    # 多 Agent 编排器（LangGraph StateGraph 13 节点）
│   └── services/
...
├── scripts/init_db.sql      # 手动建库 + 建表 SQL 脚本（含 10 个索引）
...
|------|------|------|------|------|
| GET | `/health` | 健康检查 | ✅ | 无需 |
| POST | `/api/v1/auth/login` | 认证 | ✅ | 无需 |
| POST | `/api/v1/auth/register` | 认证 | ✅ | 无需 |
| GET | `/api/v1/auth/me/{student_id}` | 认证 | ✅ | ✅ |
...
9 张表 + 10 个索引（开发阶段去掉外键约束）：

| 表名 | 用途 | 索引 |
|------|------|------|
| `students` | 学生账户（含 student_no + password_hash） | `student_no` UNIQUE |
