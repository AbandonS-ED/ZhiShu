...
│   │   ├── page.tsx                 # / 仪表盘
│   │   ├── login/                   # 登录/注册页（独立布局）
│   │   ├── duihua/ profile/ resources/ path/ tiku/ pinggu/ setting/  # 8 个学生页面
...
│   ├── src/stores/appStore.ts       # Zustand store（已接入 setting 页面）
│   ├── src/types/index.ts           # TS 类型契约（13 接口）
│   └── .npmrc                       # npmmirror 国内镜像
├── backend/                         # FastAPI + 10 表 + 8 Agent + 10 Router + 12 Service
...
│   │   ├── initial_assessment_agent.py  # 对话式 5 维画像评估（理解力/记忆力/应用转化/想象力/专注力）
...
│   ├── scripts/
│   │   ├── init_db.sql              # 10 张表 + 14 索引 + admin 账号种子数据
...
- ✅ **管理后台 9 页面** 1:1 复刻 `houtai.html` 模板 + 批量删除 + 搜索筛选 + 详情弹窗 + 题库 CRUD
- ✅ 后端完整：**10 表 + 8 Agent + 39 唯一 API 端点** + **12 Service**（含 6 个 auth 端点 + 6 个 admin 端点）
...
`student_profiles.dimensions` JSONB：`comprehension / memory / application / imagination / focus`（理解力/记忆力/应用转化/想象力/专注力），由 `initial_assessment_agent.py` 通过对话评估生成。
