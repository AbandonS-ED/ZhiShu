...
{
  "knowledge": "知识讲解（Markdown 格式，8000-15000字）：包含概念定义、核心原理、使用场景、注意事项、最佳实践、常见陷阱、性能优化、实际项目应用案例、进阶拓展",
  "code": "代码示例（含关键注释，说明时间/空间复杂度）：完整可运行的示例代码，包含多个场景的示例",
...
## 质量要求
- knowledge：内容详尽，结构清晰，有层次，适合目标学生水平，包含多个实际案例和代码片段
- code：可运行，有注释，展示核心实现，代码要实用，包含多个场景
- mermaid_code：节点 10-20 个，层次分明，不要包含特殊字符
- exercises：生成 5-8 道混合难度练习题（difficulty 30-80），选择题 4 个选项
...
## 输出格式（严格按此格式输出，只返回 JSON）
{
  "knowledge": "知识讲解（Markdown 格式，8000-15000字）：包含定义、定理、公式推导、几何直觉、典型例题、多种解法、实际应用、历史背景、拓展思考",
  "code": "不需要代码，留空字符串 \"\"",
  "mermaid_code": "思维导图的 Mermaid mindmap 代码，第一行必须是 mindmap",
  "exercises": [
    {
...
## 质量要求
- knowledge：公式清晰，推导完整，有几何直觉或实际应用背景，包含多个典型例题和变式
- code：留空字符串，数学不需要代码
- mermaid_code：节点 10-20 个，展示知识结构，不要包含特殊字符
- exercises：生成 5-8 道混合难度练习题（difficulty 30-80），包含计算题和证明题
- message：友好自然，像老师给学生讲解前的引导语
- 只返回 JSON，不要其他文字"""
...
## 输出格式（严格按此格式输出，只返回 JSON）
{
  "knowledge": "知识讲解（Markdown 格式，8000-15000字）：包含概念、原理、实验、应用、前沿发展、科学史、实际案例、跨学科联系",
  "code": "不需要代码，留空字符串 \"\"",
  "mermaid_code": "思维导图的 Mermaid mindmap 代码，第一行必须是 mindmap",
  "exercises": [
    {
...
## 质量要求
- knowledge：概念清晰，原理透彻，联系实际应用，包含多个实验案例和科学史背景
- code：留空字符串，科学理论不需要代码
- mermaid_code：节点 10-20 个，展示知识体系，不要包含特殊字符
- exercises：生成 5-8 道混合难度练习题（difficulty 30-80）
- message：友好自然，像老师给学生讲解前的引导语
- 只返回 JSON，不要其他文字"""
...
## 输出格式（严格按此格式输出，只返回 JSON）
{
  "knowledge": "知识讲解（Markdown 格式，8000-15000字）：包含概念定义、核心要点、逻辑框架、实际案例、拓展思考、历史背景、学术争议、前沿发展",
  "code": "不需要代码，留空字符串 \"\"",
  "mermaid_code": "思维导图的 Mermaid mindmap 代码，第一行必须是 mindmap",
  "exercises": [
    {
...
## 质量要求
- knowledge：结构清晰，逻辑严密，有案例支撑，包含多个实际案例和拓展思考
- code：留空字符串，理论学科不需要代码
- mermaid_code：节点 10-20 个，展示知识框架，不要包含特殊字符
- exercises：生成 5-8 道混合难度练习题（difficulty 30-80），包含理解题和应用题
- message：友好自然，像老师给学生讲解前的引导语
- 只返回 JSON，不要其他文字"""
