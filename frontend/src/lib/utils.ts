...
export function markdownToHtml(md: string): string {
  ...
  html = html.replace(/```(\w*)\n([\s\S]*?)```/g, (_, lang, code) => {
    ...
    return `<div class="codeblk"><div class="code-header"><span>${lang || '代码'}</span></div><pre>${escaped}</pre></div>`
  })
  ...
  html = html.replace(/(?:^|\n)((?:\|[^\n]+\|\n)+)/g, (match, tableBlock) => {
    ...
    headerCells.forEach((c: string) => { table += `<th style="border:1px solid #ddd;padding:6px 10px;background:#f8f8f8;text-align:left">${c.trim()}</th>` })
    table += '</tr></thead><tbody>'
    dataRows.slice(1).forEach((row: string) => {
      const cells = row.split('|').filter((c: string) => c.trim())
      table += '<tr>'
      cells.forEach((c: string) => { table += `<td style="border:1px solid #ddd;padding:6px 10px">${c.trim()}</td>` })
      table += '</tr>'
    })
    table += '</tbody></table>'
    return '\n' + table + '\n'
  ...
  html = html.replace(/^### (.+)$/gm, '<h3 style="margin:14px 0 8px;font-size:15px">$1</h3>')
  html = html.replace(/^## (.+)$/gm, '<h2 style="margin:16px 0 8px;font-size:16px">$1</h2>')
  html = html.replace(/^# (.+)$/gm, '<h1 style="margin:18px 0 10px;font-size:18px">$1</h1>')
  ...
  // 行内代码
  html = html.replace(/`([^`]+)`/g, '<code style="background:#f0f0f0;padding:1px 5px;border-radius:3px;font-size:12px">$1</code>')
  ...
  // 链接
  html = html.replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" target="_blank" style="color:var(--warm)">$1</a>')

  // 引用块
  html = html.replace(/^> (.+)$/gm, '<blockquote style="border-left:3px solid var(--warm);padding-left:10px;color:var(--ink-3);margin:8px 0">$1</blockquote>')
